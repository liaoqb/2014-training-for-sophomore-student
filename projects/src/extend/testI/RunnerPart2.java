...
public class RunnerPart2 {
    ...
    DataInputStream fin = new DataInputStream(new FileInputStream("random.txt"));
    DataInputStream countIn = new DataInputStream(new FileInputStream("count.txt"));

    int count = countIn.readInt();
    ...
    DataOutputStream countOut = new DataOutputStream(new FileOutputStream("count.txt"));

    countOut.writeInt(count);

    countOut.close();

    int[] startNodeArray = new int[26];

    for (int i = 0; i < 26; ++i) {
      startNodeArray[i] = fin.readInt();
    }
    ...
    JigsawNode startNode = new JigsawNode(startNodeArray);
