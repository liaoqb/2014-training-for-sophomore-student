...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
      ...
      // This is the code for BFSearch
    this.openList.addElement(this.beginJNode);
      // record the spring nodes
    Vector<JigsawNode> springJNodes = new Vector<JigsawNode>();
    ...
    while (!this.openList.isEmpty()) {
      ...
      springJNodes = this.findFollowJNodes(this.currentJNode);

      while (!springJNodes.isEmpty()) {
        this.openList.addElement(springJNodes.elementAt(0));
        springJNodes.removeElementAt(0);
      }
    }
	...
	private void estimateValue(JigsawNode jNode) {
		...
		for(int index =1 ; index<dimension*dimension; index++){
		  ...
		  // jNode.setEstimatedValue(3 * s + 20 * wrongDistances(jNode) + 12 * wrongCodes(jNode));
    jNode.setEstimatedValue(8 * s + 60 * wrongDistances(jNode) + 10 * wrongCodes(jNode) + 1 * getAdjacentNum(jNode));
	}

    // return the sum of the wrong distance
  private int wrongDistances(JigsawNode node) {
    int distances = 0;
    int dimension = node.getDimension();
    int x1, x2, y1, y2;

    for (int i = 1; i <= dimension * dimension; ++i) {
      x1 = (node.getNodesState()[i] - 1) / dimension;
      y1 = (node.getNodesState()[i] - 1) % dimension;
      x2 = (i - 1) / dimension;
      y2 = (i - 1) % dimension;
    ...
    // return all the wrong codes
  private int wrongCodes(JigsawNode node) {
    int sum = 0;
    int dimension = node.getDimension();
    int x1, x2, y1, y2;
  ...
  private int getAdjacentNum(JigsawNode node) {
    int sum = 0;
    int dimension = node.getDimension();

    for (int i = 1; i < dimension * dimension; ++i) {
      int lastLocation = node.getNodesState()[i];
      int nextLocation = node.getNodesState()[i + 1];

      if (lastLocation != 0 && nextLocation != 0) {
        if (lastLocation < nextLocation) {
          sum += Math.abs(nextLocation - lastLocation - 1);
        } else {
          sum += Math.abs(lastLocation - nextLocation - 1);
        }
      }
    }
